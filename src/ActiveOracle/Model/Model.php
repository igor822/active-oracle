...
class Model extends DboSource
{
    ...
    public function getQueryAdp($statement = 'query')
    {
        ...
        {
            case 'query':
                if ($this->queryAdp === null) {
                    $this->queryAdp = new Query($this->sourceModel);
                }
                return $this->queryAdp;
            break;
            case 'insert':
                if ($this->insertAdp === null) {
                    $this->insertAdp = new Insert($this->sourceModel);
                }
                return $this->insertAdp;
            break;
            case 'update':
                if ($this->updateAdp === null) {
                    $this->updateAdp = new Update($this->sourceModel);
                }
    ...
    public function find($type = 'all', $options = array())
    {
        ...
        if ($type = 'all') {
            return $this->fetchAll($this->queryAdp->getSql());
        }
    ...
    {
        if (null === $adapter) {
            $adapter = &$this->queryAdp;
        }

        if (method_exists($adapter, 'clean')) {
            $adapter->clean();
        }

        if (!empty($options['fields'])) {
            $adapter->select($options['fields']);
        } else {
            $adapter->select();
        }

        if (!empty($options['conditions'])) {
            $this->buildConditions($options['conditions'], $adapter);
        }
        if (!empty($options['order'])) {
            $adapter->order($options['order']);
        }
        if (!empty($options['group'])) {
            $adapter->group($options['group']);
        }
    ...
    */
    ...
    {
        if (null === $this->deleteAdp) {
            $this->deleteAdp = new Delete($this->sourceModel);
        }
