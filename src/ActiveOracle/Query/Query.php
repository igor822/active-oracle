...
class Query implements QueryInterface
{
    ...
    {
        $sql = $this->implodeRecursive('', $this->parts);
        $this->sql = $sql;
        return $this;
    }
        ...
        if (strpos($condition, '?') !== false) {
            ...
            if ($match) {
                ...
                for ($i = 0; $i < count($matches[0]); $i++) {
                    ...
                    if ($param instanceof \ActiveOracle\DboExpression) {
     ...
     * @return string
     */
    protected function implodeRecursive($glue, $array)
    {
        $ret = '';
        foreach ($array as $item) {
            $ret .= $glue.(is_array($item) ? $this->implodeRecursive($glue, $item) : $item);
        }
        unset($array);
        return $ret;
    }
